...
namespace DreamyTools.FarsiTools.WinControls.DrCalendar
{
    ...
    public static class FarsiDateHelper
    {
        ...
        internal static List<DayEvent> GetAllEvents()
        {           
            using (var res = Assembly.GetAssembly(typeof(FarsiDateHelper)).GetManifestResourceStream("DreamyTools.FarsiTools.WinControls.DrCalendar.Resources.DayEvents.xml"))
            {
        ...
        {
            if (!File.Exists(Application.StartupPath + "\\resources\\HijriCalendarPatch.xml")) return null;
            using (var xReader = new XmlTextReader(Application.StartupPath + "\\resources\\HijriCalendarPatch.xml"))
            {
                var xDoc = XDocument.Load(xReader);
